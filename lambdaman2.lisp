...
(define (make_move worldstate)
  ...
  (set! sleft  (penalty (score width height worldmap (- x 1) y 0 4) 3 oppdir))
  (set! sright (penalty (score width height worldmap (+ x 1) y 0 4) 1 oppdir))
  (set! sup    (penalty (score width height worldmap x (- y 1) 0 4) 0 oppdir))
  (set! sdown  (penalty (score width height worldmap x (+ y 1) 0 4) 2 oppdir))
  (debug sleft)
  (debug sright)
  (debug sup)
  (debug sdown)
